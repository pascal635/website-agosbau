...
**Nachtrag 22.09., 15:45 — Pascal hat Sitemap eingereicht und Indexierung beantragt.** Ergebnis sofort geprüft:
- **`/badsanierung-achim/`: „Gesendet und indexiert“** (Verdict PASS, gecrawlt 13:44 UTC, Canonical korrekt) — von „nicht bekannt“ zu indexiert innerhalb von Minuten. Rich-Results-Prüfung: **Rezensions-Snippet erkannt** (AggregateRating aus dem LocalBusiness-Schema) → das Schema ist für ein Rich Result *berechtigt*; ob Google es ausspielt, entscheidet sich in den SERPs. Korrigiert meine Aussage von heute Mittag („keine Rich Results“): nicht ausgespielt ≠ nicht berechtigt.
- **`/guten-fliesenleger-finden-bremen/`: „Gecrawlt – zurzeit nicht indexiert“** (Fetch erfolgreich, robots erlaubt, gecrawlt 13:44 UTC). Direkt nach dem Crawl ist das der normale Zwischenzustand — die Indexierungsentscheidung folgt in Stunden bis Tagen. **Recheck ~26.09.**; steht es nach ~2 Wochen noch so, ist es eine Qualitäts-/Prioritätsentscheidung von Google → dann Seite stärken (mehr kontextuelle Links, ggf. Inhalt vertiefen).
- **Sitemap:** eingereicht wurde `sitemap-0.xml` (Status pending, 0 Fehler). Funktioniert, weil alle 24 URLs darin liegen. Sauberer ist zusätzlich `sitemap-index.xml`, weil Astro bei Wachstum weitere Chunks anlegt, die sonst fehlen würden — optional, 10 Sekunden.
