...
- [x] **B1 · City-Page „Badsanierung Achim"** ✅ 2026-08-18 — `/badsanierung-achim/`, 866 Wörter. Trägt den **lokalen** Bezug (Sitz Auf den Mehren 45 in Bierden, alle 9 Achimer Ortsteile verifiziert, kurze Wege) und verweist für die Leistungstiefe auf `/badsanierung-bremen/`, statt sie zu duplizieren — bewusst gegen das Doorway-Risiko.
- [x] **B2 · City-Page „Fliesenleger Bremen-Neustadt"** ✅ 2026-08-18 — eigener lokaler Winkel statt Boilerplate: **Altbremer Häuser** in Buntentor, Südervorstadt und Alter Neustadt, Holzbalkendecken und Entkopplung. Ortsteile und Baucharakter vorab verifiziert. Knüpft an die Altbau-Sektion der Badsanierungsseite an.
  **Nebenbefund behoben:** Die Ortsliste auf der Startseite war **hartcodiert** — Neustadt wäre unverlinkt geblieben. Sie wird jetzt aus `cities.ts` abgeleitet, künftige Stadtteilseiten erscheinen automatisch. `/badsanierung-achim/` war ebenfalls verwaist und ist nun über ein optionales `related`-Feld in `cities.ts` von `/fliesenleger-achim/` sowie von `/badsanierung-bremen/` verlinkt.
...
**GSC per MCP nur lesend** (Full User, kein Owner): Sitemap-Submit und „Indexierung beantragen“ laufen über die GSC-Oberfläche (Pascal).
**Indexierung 22.09. (nach manuellem Antrag):** Badsanierung Achim indexiert 22.09. ✅ (Rezensions-Snippet berechtigt) · Ratgeber „gecrawlt, zurzeit nicht indexiert“ → Recheck ~26.09. · Sitemap `sitemap-0.xml` eingereicht (pending), `sitemap-index.xml` optional nachreichen.
